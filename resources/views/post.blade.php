<x-layout>
	...
	{{-- <article class="py-8 max-w-screen-md">
		...
		<div>
			By
			<a class="hover:underline font-normal text-slate-500" href="/authors/{{ $post->author->username }}">{{ $post->author->name }}</a>
			in
  ...
  <div class="flex justify-between px-4 mx-auto max-w-screen-xl ">
      <article class="mx-auto w-full max-w-4xl format format-sm sm:format-base lg:format-lg format-blue dark:format-invert">
          <header class="mb-4 lg:mb-6 not-format">
						<a href="/posts" class="text-blue-500 text-xs">&laquo; Back to All Posts</a>
						<address class="flex items-center my-6 not-italic">
								<div class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white">
										<img class="mr-4 w-16 h-16 rounded-full" src="https://flowbite.com/docs/images/people/profile-picture-2.jpg" alt="Author Name">
										<div>
												...
												<a href="/categories/{{ $post->category->slug }}">
													<p class="bg-{{ $post->category->color }}-100 text-base text-gray-500 dark:text-gray-400">{{ $post->category->name }}</p>
												</a>
										</div>
								</div>
						</address>
